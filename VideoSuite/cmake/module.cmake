...
macro(CreateTarget ProjectName Type)
    ...
    # 添加自定义库到目标文件的链接库中	
	foreach(_lib ${SELF_LIBRARY_LIST_DEBUG})
		target_link_libraries(${PROJECT_NAME} debug ${_lib})
	endforeach()

	foreach(_lib ${SELF_LIBRARY_LIST_RELEASE})
		target_link_libraries(${PROJECT_NAME} optimized ${_lib})
	endforeach()
	
	# 链接其他文件
	foreach(_lib ${OTHER_LIBRARY_LIST_DEBUG})
        target_link_libraries(${PROJECT_NAME} debug ${_lib})   # 链接库文件
    endforeach()
	
	foreach(_lib ${OTHER_LIBRARY_LIST_RELEASE})
        target_link_libraries(${PROJECT_NAME} optimized ${_lib})   # 链接库文件
    endforeach()
